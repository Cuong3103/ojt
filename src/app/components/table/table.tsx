        ...
        return (
          <span>
            <Chip
              ...
              style={{
                backgroundColor: row[column.id] === USER_ROLE.ADMIN ? "#4db848" : "",
              }}
            />
          </span>
        );
        ...
        return (
          <span>
            {Array.isArray(row[column.id]) && row[column.id].length > 0 ? (
              row[column.id].map((output: string, index: number) => <Chip key={index} active={output} />)
            ) : (
              <Chip active="H4SD" />
            )}
          </span>
    ...
    <div className="bg-white shadow-md overflow-x-auto rounded-lg h-3/5 m-4 w-full">
      <table className=" w-full">
        <thead className="bg-primary-color text-white sticky top-0">
          <tr>
            {columns.map((column) => (
              <th className={`px-6 py-3 text-left text-xs  uppercase tracking-wider`} key={column.id}>
        ...
        <tbody className="divide-y divide-black w-full overflow-y-scroll">
          {data.map((row, index) => (
            <tr key={index} className="hover w-1/4">
              {columns.map((column) => (
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900" key={column.id}>
